					...
					if (err) {
						...
						if (other_user) {
							if (user.favorites_by_category[_category]) {
								if (!idInArray(other_user._id.toString(), user.favorites_by_category[_category])) {
				...
				if (user) {
					User.findOne({username: _other_username}, "_id", function(err, other_user) {
						if (err) {
							...
							if (other_user) {
								if (user.favorites_by_category[_category]) {
									var index = idIndexInArray(other_user._id.toString(), user.favorites_by_category[_category]);
									if (index >= 0) {
